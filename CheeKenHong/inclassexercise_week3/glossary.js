...
// create h2 heading - filter by class
const titleFilter = document.createElement("label")
titleFilter.innerHTML = "Filter by Class: "

// create dropdown menu
const dropDownList = document.createElement("select")

// filter those class that are unique
let unique = [...new Set(glossary.map(item => item.class))];
// unique.includes('')
// console.log(unique);
unique.unshift('')
...
document.body.append(titleFilter, dropDownList)

const break1 = document.createElement("br")
document.body.append(break1)

const tagsFilter = document.createElement("label")
tagsFilter.innerHTML = "Filter by tags: "
document.body.append(tagsFilter)
const break2 = document.createElement("br")
document.body.append(break2)

// create checkbox by tagging
let uniqueTag = []
let testTag = glossary.forEach((item) => {
    item.tags.forEach((tagsItem) => {
        if (uniqueTag.includes(tagsItem) === false) {
            uniqueTag.push(tagsItem)

            let newCheckbox = document.createElement("input")
            newCheckbox.type = "checkbox"
            newCheckbox.value = tagsItem
            document.body.append(newCheckbox)

            let newLabel = document.createElement("label")
            newLabel.innerHTML = tagsItem
...
const titleFilter2 = document.createElement("label")
titleFilter2.innerHTML = "Filter by Search Key : "
document.body.append(titleFilter2)

const newInput = document.createElement("textarea")
document.body.append(newInput)

const newButton = document.createElement("button")
newButton.innerText = "Press to search"
...
function handleOnClick(event) {
    ...
    // initialize newDiv everytime
    newDiv.innerHTML = ''
    let existUL = document.querySelector("ul")
    console.log(existUL)
    if (existUL != null) {
        existUL.remove()
    }
    ...
    // selected option
    let selectedClass = document.querySelector("select")
    //console.log(selectedClass.options[selectedClass.selectedIndex].value)
    let classKey = selectedClass.options[selectedClass.selectedIndex].value
    console.log("Classkey: " + classKey)

    // filter by class, given that ddl is isselected
    let filter1 = glossary
    if (classKey != '') {
        filter1 = glossary.filter(item => item.class === parseInt(classKey))
        //console.log(filter1)
    }

    // filter by tags
    let allInput = document.querySelectorAll("input")
    allInput.forEach((inputChecked) => {
        if (inputChecked.type == "checkbox" && inputChecked.checked == true) {
            filter1 = filter1.filter(item => item.tags.includes(inputChecked.value))
            //console.log("filtering: " + inputChecked.value)
        }
    })

    // search key
    let searchKey = document.querySelector("textarea")
    console.log("Searchkey: " + searchKey.value)

    // filter by search Key
    if (searchKey.value != '') {
        filter1 = filter1.filter(item => item.definition.includes(searchKey.value))
    }
    ...
    filter1
        .forEach((item) => {
            ...
            newList.innerHTML += item.class + " - " + item.term + " , " + item.definition
...
newButton.addEventListener("click", handleOnClick);

// let singleChecked = 'DOM'
// let secondChecked = 'Events'
// let multiChecked = ['DOM', 'Browser']
// // let filterGloss = glossary.filter(item => item.class === 6 && item.definition.includes("as") && item.tags.includes(test))
// let filterGloss = glossary
//     .filter(item => item.tags.includes(singleChecked))
//     .filter(item => item.tags.includes(secondChecked))
// console.log(filterGloss)

// let multiGloss = glossary.filter(item => multiChecked.filter(checked => item.tags.includes(checked)))
// console.log(multiGloss)

// 1. To iterate thru the array and display in HTML element
// 2. Include textbox to allow user to key in any definition keyword
// 3. Include dropdown with all the class name, include this as part of the search
// 4. Include all possible tags using checkbox, include this as part of the search

// For item 3 and 4, not sure whether am I doing according to the question, please advice if I'm incorrect
